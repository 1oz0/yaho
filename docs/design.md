...
| LLM 호출 | **전면 금지** (§11) | **여행코스 생성에 한해 허용** | 추가 요청. 분류·페르소나·예산·진척은 규칙 기반 유지 — 아래 §10 참조 |
...
## 10. Claude API 연동 — 왜 여기까지만인가 (추가 요청 1)

프롬프트 §11 은 "LLM / 외부 AI API 호출" 을 금지 항목으로 두었고, 평가기준 ③ 은
"핵심 계산 로직이 AI 없이 결정론적으로 동작하는가" 다. 추가 요청으로 **여행코스 생성에만**
Claude 를 쓰기로 했으므로, 이 둘이 충돌하지 않는 경계를 코드 구조로 못박았다.

### 10-1. 경계선

| 기능 | AI | 근거 |
|---|---|---|
| 거래 분류 (329 키워드 + 54 MCC + 정기결제 탐지) | ❌ | 거래 건수에 비례해 호출이 늘어난다. 사용자가 늘어도 비용이 늘지 않는 구조여야 한다 |
| 페르소나 산출 (카테고리 비중 × 시간대 비중) | ❌ | 같은 입력에 같은 결과가 나와야 하고, 근거 수치를 화면에 그대로 노출한다 |
| 절약 목표 · 주차 예산 · 진척률 | ❌ | 금액 계산. 합계가 1원이라도 어긋나면 안 된다 |
| 여행지 후보 선정 · 블러 정책 | ❌ | 금액 비교와 산술 |
| **여행코스의 장소 선택 · 순서 · 문구** | ✅ | 사용자당 한 번, 페르소나 맥락을 반영한 판단이 필요한 유일한 지점 |

호출 빈도가 결정적이다. 분류는 거래 421건마다, 코스 생성은 (사용자 · 여행지 · 페르소나)당
한 번이다. 후자만 AI 로 돌리면 사용자가 늘어도 비용이 선형으로만 늘고, 캐시가 그마저 줄인다.

의존 방향도 한 곳으로 제한했다. `AiModule` 을 import 하는 것은 `TravelModule` 뿐이며,
`AnalysisModule` / `PersonaModule` / `ChallengesModule` 은 Claude 를 알지 못한다.

### 10-2. AI 가 숫자를 쓰지 못하게 한 이유
